					...
					echo '<div class="my_account_page_content_wrapper">';
						echo '<div class="my_account-content-inner">';
							...
							if(isset($_GET['inner_page']) && $_GET['inner_page'] == 'posts-wishlist'){ 
								do_action('boliin_get_all_posts_wishlist');
							}else{
			...
			echo '<div class="my_account_page_content_wrapper login-register-page">';
				...
				if(isset($_GET['action']) && $_GET['action'] == 'register'){
					...
					if(!get_option( 'users_can_register' )){
						$html = '<div class="alert alert-info">' . esc_html__('Website does not allow register', 'boliin') . '</div>';
					}
					$register_image = BOLIIN_THEME_URL . '/assets/images/register.png';
					$image_options = boliin_get_option('register_image', array('id'=> 0));
					if(isset($image_options['url']) && $image_options['url']){
      				$register_image = $image_options['url'];
   				}

   				$login_link = home_url('/wp-login.php?action=login&redirect_to=' . get_permalink());
		         if(class_exists('BABE_Settings')){
							...
							echo '<div class="row">';
								...
								echo '
									<div class="col-12 col-md-5 register-content-left">
										<span class="img-register">
											<img src="' . esc_url($register_image) . '" alt="' . esc_attr__('Register', 'boliin') . '">
										</span>
										<div class="content-inner">
											<div class="quick-login">
												<span class="text">' . esc_html__('Already a member', 'boliin') . '</span>
												<a class="btn-theme btn-small login-link" href="' . esc_url($login_link) . '">
													' . esc_html__('Login', 'boliin') . '             
												</a>
											</div>
										</div>
									</div>
								...
								echo '
									<div class="col-12 col-md-7 register-form-content">
										<h3 class="title">' . esc_html__('Create a free account', 'boliin') . '</h3>
										<div class="desc"> ' . esc_html__('A few clicks away from creating your account', 'boliin') . '</div>
					...
					$register_link = home_url('/wp-login.php?action=register');
				   if(class_exists('BABE_Settings')){
					...
					echo '<div class="z-login-form">';
						echo '<div class="form-content">';
							...
							echo '<div class="user-registration">';
								echo esc_html__('Do not have an account', 'boliin');
								echo '<a class="quick-login-link" href="' . $register_link . '">' . esc_html__('Register', 'boliin') . '?</a><br>';
								echo '<a class="lost-password" href="' . $lostpassword_link . '">' . esc_html__('Forgot your password?', 'boliin') . '</a>';
							echo '</div>';
						echo '</div>';   
					echo '</div>';   
				}
