<?php
class Boliin_BA_DashBoard{

	public function __construct(){
		add_filter('babe_myaccount_manager_all_posts', array($this, 'get_all_posts_html'), 1, 3);
		add_action('boliin_get_all_posts_wishlist', array($this, 'get_all_posts_wishlist'));
	}
	...
	public static function get_all_posts_html($output, $post_type, $user_info){
		...
		if($output){
			$output = '
		 		<div class="my_account_all_posts_total">'.esc_html__( 'Total: ', 'boliin' ).$found_posts.'</div>    
		 		<table class="my_account_all_posts_table">
				 	<thead>
				 		<tr>
				 			<td> '.esc_html__('Title & Image', 'boliin').' </td>
				 			<td> '.esc_html__('Published Date', 'boliin').' </td>
				 			<td> '.esc_html__('Action', 'boliin').' </td>
				 		</tr>
				 	</thead>			
			  		<tbody>
			 			'.$output.'
  	...
  	public function get_all_posts_wishlist(){
		...
		$args = array(
	 	...
	 	while ( $the_query->have_posts() ) : $the_query->the_post();
		 	...
		 	$output .= '
		 		<tr>
			 		...
			 		<td class="my_account_all_posts_td my_account_all_posts_td_date">' . get_the_date('', $post_id) . '</td>
			 		<td class="my_account_all_posts_td my_account_all_posts_td_action">' . Boliin_Addons_Wishlist_Ajax::html_icon($post_id) . '</td>
		 		</tr>';
	 	endwhile;
	 	wp_reset_postdata();
	 	...
	 	if($output){
			$output = '
		 		<div class="my_account_all_posts_total">'.__( 'Total: ', 'boliin' ).$found_posts.'</div>    
		 		<table class="my_account_all_posts_table">
				 	<thead>
				 		<tr>
				 			<td> '.esc_html__('Title & Image', 'boliin').' </td>
		...
		$output = '
			<div class="my_account_inner_page_block my_account_all_posts">
			  	<h2>'.esc_html__('Wishlist', 'boliin').'</h2>
			  	<div class="my_account_all_posts_inner">
					'.$output.'
					'.BABE_Functions::pager($max_num_pages).'
			 	</div>
